...
# Lista de servidores
servers = [("192.168.11.139", 9001), ("192.168.11.139", 9002), ("192.168.11.139", 9003)]  # Ips de onde estao os servidores, onde eles ficam ouvindo [IP_OndeServOuve]
IP_OndePortalOuve = ['192.168.11.139']
...
def tratarCliente(client_socket, mode):
    global RR
    ...
    try:
        ...
        enviar_string(client_socket.getpeername()[0], 8002, resultado_final)
        client_socket.close()

def client_thread(client_socket, mode):
    tratarCliente(client_socket, mode)
...
def main():
    ...
    # Inicia socket e fica aguardando uma conexão de cliente
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        while True:
            try:
                ...
                threading.Thread(target=client_thread, args=(client_socket, mode)).start()
