...
# Lista de servidores
servers = [("192.168.11.107", 9001), ("192.168.11.107", 9002), ("192.168.11.107", 9003)] #Ips de onde estao os servidores, onde eles ficam ouvindo [IP_OndeServOuve]
IP_OndePortalOuve = ['192.168.11.137']
IP_PORTA_OndeClienteOuve = ['192.168.11.107', 8002] 
...
def receber_resultado(ip, port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as recebe:
        ...
        recebe.bind((IP_OndePortalOuve[0], port)) #Precisa ser a mesma porta que na linha 68 do codigo do servidor (IP onde esta o IP_PORTA_OndePortalOuve)
        recebe.listen(1)
        resultado = ""
...
def handle_client(client_socket, mode, RR):
    try:
        ...
        enviar_string(IP_PORTA_OndeClienteOuve[0], IP_PORTA_OndeClienteOuve[1], resultado_final) #ip de onde o cliente esta ouvindo
        return RR
...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        server_socket.bind((IP_OndePortalOuve[0], port)) #ip de onde esta o portal 
        ...
        while True:
            try:
                ...
                RR = handle_client(client_socket, mode, RR)
